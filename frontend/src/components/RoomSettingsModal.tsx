    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-main">
          <div>
            <h2 className="text-text-main font-bold text-[16px]">Oda Ayarları</h2>
            <p className="text-text-muted text-[12px] mt-0.5">{roomName}</p>
          </div>
        ...
        <div className="p-5 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          {/* Açıklama */}
          <div>
            <label className="text-[12px] font-semibold uppercase tracking-wider text-text-muted">Açıklama</label>
            <textarea
              value={description}
              maxLength={200}
              ...
              className="mt-2 w-full bg-white/[0.03] border border-border-main rounded-xl px-3 py-2.5 text-text-main text-sm placeholder:text-text-muted outline-none focus:border-primary-main/50 transition-colors resize-none"
            />
            <div className="flex items-center justify-between mt-2">
              <span className="text-[11px] text-text-muted">{description.length}/200</span>
              <button
                onClick={saveDescription}
                disabled={saving || description.trim() === initialDescription.trim()}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-sm font-semibold text-text-main transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            <p className="text-[11px] text-text-muted mt-1">Oda adı değiştirilemez (mesaj geçmişine bağlıdır).</p>
          </div>

          {/* Yasaklılar */}
          <div>
            <label className="text-[12px] font-semibold uppercase tracking-wider text-text-muted flex items-center gap-1.5">
              <Ban size={13} /> Yasaklılar {!loadingBans && `(${bans.length})`}
            </label>
            <div className="mt-2 space-y-1.5">
              {loadingBans ? (
                <div className="text-center py-4 text-text-muted text-[12px] flex items-center justify-center gap-2">
                  <Loader2 size={14} className="animate-spin" /> Yükleniyor...
                </div>
              ) : bans.length === 0 ? (
                <div className="text-center py-4 text-text-muted text-[12px]">Yasaklı kullanıcı yok.</div>
              ) : (
                bans.map((b) => (
                  <div key={b.userId} className="flex items-center gap-2.5 p-2 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                    <div className="w-8 h-8 rounded-full bg-bg-base flex items-center justify-center text-lg overflow-hidden flex-shrink-0">
                      {renderAvatar(b.avatarId || 'default')}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-text-main text-[13px] font-medium truncate">{b.username}</div>
                      {b.reason && <div className="text-text-muted text-[11px] truncate">{b.reason}</div>}
                    </div>
                    <button
